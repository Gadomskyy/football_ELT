...
@dag(
    dag_id="football_data_pl_teams_bronze",
    description=(
        """
        Loads Premier League teams from the football-data.org API into a BigQuery table in the bronze layer.
...
def football_data_pl_teams_bronze():

    @task(task_id="load_pl_teams_to_bronze")
    def load_teams(season:int) -> dict:
        api_key = os.getenv(
    ...
    load_teams.expand(season=SEASONS)

dag = football_data_pl_teams_bronze()